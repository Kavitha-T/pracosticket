...
if($cfg->showAnsweredTickets()) {
    ...
    if ($stats) {
        ...
        $nav->addSubMenu(array('desc'=>$open_name.' ('.number_format($stats['open']).')',
                               'title'=>__('Open Tickets'),
                               'href'=>'testingtickets.php?status=open&statusid=1',
                               'iconclass'=>'Ticket'),
                            (!$_REQUEST['status'] || $_REQUEST['status']=='open'));
...
if($thisstaff->showAssignedOnly() && $stats['closed']) {
    ...
    $nav->addSubMenu(array('desc' => __('Closed').' ('.number_format($stats['closed']).')',
     ...
     $nav->addSubMenu(array('desc'=>('Waiting on Dev').'('.number_format($stats['Waiting on dev']).')',
                               'title'=>__('Waiting on dev'),
                               'href'=>'testingtickets.php?status=waitingondev&statusid=7',
                               'iconclass'=>'Ticket'),
                            (!$_REQUEST['status'] || $_REQUEST['status']=='open'));
                            
        
        $nav->addSubMenu(array('desc'=>('Waiting on Customer').' ('.number_format($stats['Waiting on customer']).')',
                               'title'=>__('Waiting on Customer'),
                               'href'=>'testingtickets.php?status=waitingoncust&statusid=6',
                               'iconclass'=>'Ticket'),
                            (!$_REQUEST['status'] || $_REQUEST['status']=='open'));
